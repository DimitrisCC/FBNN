...
parser = argparse.ArgumentParser('ConvClassification')
...
str_ = 'conv_classification/{}/NA{}_LR{}_AI{}/{}_'.format(
...
def run():
    ...
    x_train_flat = data.x_train_flat[permutation[n_test:]]
    x_valid, y_valid = data.x_train[permutation[:n_test]], data.y_train[permutation[:n_test]]
    x_valid_flat = data.x_train_flat[permutation[:n_test]]
    ...
    lower_ap, upper_ap = np.min(x_train_flat, 0), np.max(x_train_flat, 0)
    ...
    layer_sizes = [in_channels] + [5]*args.n_layers + [num_classes]

    model = EntropyEstimationFVI(
        kern, get_posterior('cbnn', classification=True)(layer_sizes, logstd_init=-5., img_shape=img_shape),
    ...
    inducing_points = x_train_flat if args.inducing_points == 0 \
                else x_train_flat[np.random.choice(x_train.shape[0], replace=False, size=args.inducing_points), :]
    ...
    for epoch in range(epochs):
        ...
        for idx in range(iter):
            ...
            x_batch, y_batch = x_train_flat[sub_indices, :], y_train[sub_indices].astype(np.int32)
            _, loss = sess.run(
                [model.infer_gp, model.gp_loss],
        ...
        for id in range(test_input.shape[0] // 1000 + 1):
            ...
            if for_gp:
                acc, l = sess.run([
                    ...
                    feed_dict={model.x_gp: x_train_flat, model.y_gp: y_train,
    ...
    gp_acc, gp_logll = eval(x_test_flat, y_test, for_gp=True)
    ...
    for epoch in range(epochs):
        ...
        for idx in range(iter):
            ...
            x_batch, y_batch = x_train_flat[sub_indices, :], y_train_onehot[sub_indices, :]
        ...
        valid_acc, valid_likelihood = eval(x_valid_flat, y_valid_onehot)
        if valid_likelihood > best_valid_likelihood:
            ...
            test_acc, test_likelihood = eval(x_test_flat, y_test_onehot)
