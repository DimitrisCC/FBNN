...
class FunctionalBNNModel(BayesianNN):
    ...
    def __init__(self, hparams, name="FBNN"):
        ...
        if self.hparams.use_sigma_exp_transform:
            ...
            self.inverse_sigma_transform = lambda y: y + \
                np.log(1. - np.exp(-y))
    ...
    def build_model(self, activation_fn=tf.nn.relu):
        ...
        for l_number, n_nodes in enumerate(self.layers):
            if n_nodes > 0:
                h = self.build_layer(
                    input_x, [n_in, n_nodes], l_number, self.hparams.activation)
                input_x = h
                n_in = n_nodes
        ...
        # (h - tf.to_float(self.gp.input_mean)) / tf.to_float(self.gp.input_std)
        self.func_x_adv = h
        self.func_x = self.func_x_adv[:, :tf.shape(self.x)[0]]
        self.func_adv = self.func_x_adv[:, tf.shape(self.x)[0]:]
        self.y_pred = h[0, :tf.shape(
            self.x)[0]] * tf.to_float(self.gp.input_std) + tf.to_float(self.gp.input_mean)
        #TODO: mean

        self.injected_noise = 0.01
        noisy_func_x_adv = self.func_x_adv + \
            self.injected_noise * tf.random_normal(tf.shape(h))
        tmp = tf.boolean_mask(tf.transpose(
            noisy_func_x_adv, [1, 2, 0]), self.weights_x_adv > 0)
        ...
        y_normed = (self.y - tf.to_float(self.gp.input_mean)) / \
            tf.to_float(self.gp.input_std)
        log_likelihood = log_gaussian(
            y_normed, self.func_x, self.obs_sigma, reduce_sum=False)
        ...
        # tf.minimum(tf.to_float(self.global_step % 20000) / 15000., 1.) #TODO
        kl_coeff = 1.
        elbo = self.weighted_log_likelihood - kl / batch_size * kl_coeff

        self.loss = -elbo
        vars = list(set(tf.trainable_variables()) -
    ...
    def build_graph(self, graph, sess):
        ...
        with self.graph.as_default():
            ...
            self.weights = tf.placeholder(
                shape=[None, self.n_out], dtype=tf.float32)

            xmin, xmax = tf.reduce_min(self.x, 0), tf.reduce_max(
                self.x, 0)  # TODO: adv region
            self.adv = tf.random_uniform(
                shape=[self.n_adv, self.n_in]) * (xmax-xmin) * 2 + xmin - (xmax-xmin)/2.
            rand = tf.to_int32(tf.random_uniform(
                shape=[self.n_adv], minval=-0.5, maxval=self.n_out-0.5))
            self.weights_adv = tf.one_hot(rand, self.n_out)

            self.x_adv = tf.concat([self.x, self.adv], axis=0)
            self.weights_x_adv = tf.concat(
                [self.weights, self.weights_adv], axis=0)
            ...
            for step in range(num_steps):
                ...
                _, global_step, loss, ll, obss, yp = self.sess.run(
                    [self.train_op, self.global_step, self.loss,
                        self.weighted_log_likelihood, self.obs_sigma, self.y_pred],
                    feed_dict={
                        self.x: x,
                        self.y: y,
                ...
                losses.append(loss)
                if step % 100 == 0:
                    print('Iter %d/%d -- ELBO %.4f -- Log Likelihood %.4f -- Obs Sigma %.4f' %
                          (step, num_steps, -loss, ll, obss))
                    print('Y Pred = {}'.format(np.choose(weights.argmax(
                        1), yp.transpose()).reshape([1, -1])[0, :5]))
                    print('Y True = {}'.format(np.choose(weights.argmax(
                        1), y.transpose()).reshape([1, -1])[0, :5]))
