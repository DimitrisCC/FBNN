...
class BBAlphaDivergence(BayesianNN):
    ...
    def initialize_model(self):
        ...
        with self.graph.as_default():
            ...
            self.data_size = tf.placeholder(
                tf.float32, shape=(), name='data_size')
    ...
    def build_layer(self, input_x, shape, layer_id, activation_fn=tf.nn.relu):
        ...
        w_sigma = self.sigma_transform.forward(
            self.build_sigma_variable(shape))
    ...
    def sample_neural_network(self, activation_fn=tf.nn.relu):
        ...
        with self.graph.as_default():
            ...
            for layer_id in range(self.total_layers):
                ...
                t2 = (0.5 * w ** 2 / n) * \
                    (1 / self.prior_variance - 1 / w_sigma ** 2)
                log_f += tf.reduce_sum(t1 + t2)

                t1 = b * b_mu / (n * b_sigma ** 2)
                t2 = (0.5 * b ** 2 / n) * \
                    (1 / self.prior_variance - 1 / b_sigma ** 2)
                log_f += tf.reduce_sum(t1 + t2)
    ...
    def log_z_q(self):
        ...
        with self.graph.as_default():
            ...
            for layer_id in range(self.total_layers):
                ...
                w_term += 0.5 * \
                    tf.reduce_sum(tf.log(2 * np.pi) + 2 * tf.log(w_sigma))

                b_term = 0.5 * tf.reduce_sum(b_mu ** 2 / b_sigma ** 2)
                b_term += 0.5 * \
                    tf.reduce_sum(tf.log(2 * np.pi) + 2 * tf.log(b_sigma))
